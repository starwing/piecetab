...
static void ptM_sethole(pt_Node *n, int i, int h) {
    assert(i >= 0 && i < PT_FANOUT), h = h ? 1 : 0;
...
static void ptN_remove(pt_State *S, pt_Node *p, int k, int s, int e) {
    assert(s <= e && e <= ptN_cc(p));
    ptN_purge(S, p, k, s, e, p->version), ptN_move(p, s, e, ptN_cc(p) - e);
...
static void ptN_makespace(pt_Node *p, int i, int n) {
    ...
    ptN_move(p, i + n, i, ptN_cc(p) - i), ptN_setcc(p, ptN_cc(p) + n);
}
...
PT_API pt_Alloc *pt_getallocf(pt_State *S, void **pud)
{ if (S == NULL) return NULL; if (pud) *pud = S->ud; return S->allocf; }
/* clang-format on */
...
PT_API pt_Buffer pt_from(pt_State *S, const char *s, size_t len) {
    ...
    memset(t, 0, sizeof(pt_Tree)), t->S = S, t->refc = 1, t->from = &S->empty;
    if (len == 0) return t;
    t->root.children[0] = (pt_Node *)s;
    t->root.bytes[0] = len, t->bytes = len, ptN_setcc(&t->root, 1);
    return (t->root.version = ++S->max_version), t;
}
...
PT_API unsigned pt_release(pt_Buffer b) {
    ...
    for (;;) {
        ...
        ptA_destroy(t->S, &t->arena), ptP_free(&t->S->trees, t);
...
static int ptK_seamleaf(pt_Cursor *C, int right) {
    int      i, l = ptK_levels(C);
    pt_Node *p;
    size_t   n;
    assert(right >= 0), i = ptK_idx(C, p = ptK_parent(C, l), l) + right;
    if (i < 1 || i >= ptN_cc(p)) return 0;
    if (ptM_ishole(p, i - 1) || ptM_ishole(p, i)) return 0;
    if (ptN_lit(p, i - 1) + p->bytes[i - 1] != ptN_lit(p, i)) return 0;
    n = p->bytes[i - 1], p->bytes[i - 1] += p->bytes[i];
    if (assert(C->paths[l] <= &p->children[i]), C->paths[l] == &p->children[i])
        C->off -= n, C->poff += n, C->paths[l] = &p->children[i - 1];
    return ptN_remove(C->tree->S, p, 0, i, i + 1), 1;
}
...
static void ptI_splitchild(pt_Cursor *C, int l) {
    ...
    assert(mid >= PT_FANOUT / 2), ptN_setcc(nd, mid), ptM_clamp(nd);
...
static void ptI_fillrt(pt_Cursor *C, const char *s, size_t len, int h) {
    ...
    if (po == 0 || po >= n) return;
    nn = (pt_Node *)(ptN_lit(p, i) + po);
    if (ph) nn = ptH_new(C->tree->S, ptN_hole(p, i)->data + po, rm);
    if (!h && !ph && s + len == ptN_lit(p, i) + po)
        rt->bytes[0] += rm;
    else {
...
static void ptI_splitins(pt_Cursor *C, const char *s, size_t len, int h) {
    ...
    int      cc, need, m, i, l = ptK_levels(C);
    size_t   n, po = C->poff;
    i = ptK_idx(C, p = ptK_parent(C, l), l), n = p->bytes[i], cc = ptN_cc(p);
    assert(po <= n), rt->mask = 0, rt->version = C->tree->root.version;
    ptI_fillrt(C, s, len, h), i = ptK_idx(C, p, l), need = ptN_cc(rt);
    ...
    if (po == n)
        C->poff = len, ptK_seamleaf(C, 1);
    else
        (need == 1 && po > 0) ? (C->poff = len) : (C->off += len, C->poff = 0);
}
...
static void ptD_seambound(pt_Cursor *C, int l, pt_Node **ns, int *cL, int *cR) {
    pt_Node *p = ptK_parent(C, l);
    int      i = (int)(ns - p->children);
    size_t   n, nb;
    *cL = ptN_cc(ns[0]), *cR = ptN_cc(ns[1]);
    if (l != ptK_levels(C) - 1 || *cL == 0 || *cR == 0) return;
    if (ptM_ishole(ns[0], *cL - 1) || ptM_ishole(ns[1], 0)) return;
    if (ptN_lit(ns[0], *cL - 1) + ns[0]->bytes[*cL - 1] != ptN_lit(ns[1], 0))
        return;
    if (*C->paths[l] == ns[0]) {
        nb = ns[1]->bytes[0], ns[0]->bytes[*cL - 1] += nb;
        p->bytes[i] += nb, p->bytes[i + 1] -= nb;
        ptN_remove(C->tree->S, ns[1], 0, 0, 1), *cR -= 1;
    } else {
        ...
        *cL -= 1, ptN_setcc(ns[0], *cL), ptM_clamp(ns[0]);
    }
}
...
static int ptD_foldnode(pt_Cursor *C, int lfirst, int l) {
    ...
    int       r = 0, cL, cR, dn, i = ptK_idx(C, p, l);
    pt_Node **ns = &p->children[i];
    pt_Delta  ds;
    if (assert(ptN_cc(p) > 1), ptN_cc(ns[0]) > PT_FANOUT / 2) return 0;
    assert(ns[0]->version == C->tree->root.version);
    if ((i && lfirst) || i == ptN_cc(p) - 1) ns -= 1, i -= 1, r = 1;
    ns[!r] = ptK_cow(C, l, r ? -1 : 1);
    if (ptD_seambound(C, l, ns, &cL, &cR), cL + cR <= PT_FANOUT) {
        ptN_copy(ns[0], cL, ns[1], 0, cR);
        ptN_setcc(ns[0], cL + cR), ptN_setcc(ns[1], 0);
    ...
    dn = ptD_balancenode(ns, !r, &ds), assert(dn != 0 && (dn < 0) != r);
    if (r) cp[1] += dn;
    ptM_sethole(p, i, (int)ns[0]->mask);
    ptM_sethole(p, i + 1, (int)ns[1]->mask);
    return (p->bytes[i] -= ds, p->bytes[i + 1] += ds), 0;
}
...
static void ptD_rebalance(pt_Cursor *C, int l) {
    ...
    for (; l > 0; --l) {
        ...
        if (assert(ptN_cc(p) > 1), !ptD_foldnode(C, 0, l)) return;
    }
    ...
    while (ptK_levels(C) && ptN_cc(&C->tree->root) == 1) {
        int i = ptK_idx(C, p = ptK_parent(C, 1), 1);
        C->tree->root = *p, ptP_free(&C->tree->S->nodes, p);
...
static void ptD_backwardnode(pt_Cursor *C, int d, int l) {
    pt_Node *p;
    int      dl, i = ptK_idx(C, p = ptK_parent(C, l), l);
    if (d > i) {
        d -= i + 1, dl = l;
...
static int ptD_makechain(pt_Cursor *C, int from, int to, int nofail) {
    ...
    if (assert(from < to), from < 0) {
        assert(to == ptK_levels(C)), p = &C->tree->root;
        h = (p->mask & ptM_mask(ptN_cc(p))) != 0;
...
static void ptD_findroom(pt_Cursor *C, int l) {
    ...
    assert(fl >= 0 && ptN_cc(p) - i - 1 == 0), ptD_makechain(C, fl, l, 1);
}
...
static void ptD_stitch(pt_Cursor *L, pt_Node *rt) {
    ...
    if (d > L->poff) {
        ptD_backwardnode(L, 1, l), p = ptK_parent(L, l), i = ptK_idx(L, p, l);
...
static void ptD_cutpiece(pt_Cursor *C, size_t lo, size_t hi) {
    ...
    if (ptM_ishole(p, i)) {
        ...
        if (p->bytes[i] == 0)
            ptN_remove(C->tree->S, p, 0, i, i + 1), ptK_seamleaf(C, 0);
        return;
    }
    if (lo == 0 && hi == p->bytes[i])
        ...
        p->bytes[i] = p->bytes[i] - hi, ptK_seamleaf(C, 0);
    } else if (hi == p->bytes[i])
        p->bytes[i] = lo, ptK_seamleaf(C, 1);
    else {
...
static void ptD_rmleaf(pt_Cursor *C, size_t del) {
    ...
    if (ptN_cc(p) < oc && l > 0) ptD_rebalance(C, l - 1), l = ptK_levels(C);
    p = ptK_parent(C, l), i = ptK_idx(C, p, l);
    if (C->poff == p->bytes[i] && doff < ptK_bytes(C)) ptK_forwardoff(C, 0);
}
...
PT_API int pt_remove(pt_Cursor *C, size_t len) {
    ...
    for (dl = 0; dl < ptK_levels(C) && C->paths[dl] == R.paths[dl]; ++dl) {
        i = ptK_idx(&R, ptK_parent(&R, dl + 1), dl + 1);
        R.paths[dl + 1] = &ptK_cow(C, dl, 0)->children[i];
    }
    for (l = dl; dl < ptK_levels(C); ++dl) ptK_cow(C, dl, 0);
    for (dl = l; dl < ptK_levels(&R); ++dl) ptK_cow(&R, dl, 0);
    l += (l == ptK_levels(C) && C->paths[l] == R.paths[l]);
    if (l <= ptK_levels(C)) return ptD_rmrange(C, &R, l), PT_OK;
