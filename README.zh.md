...
两个轻量级 stb 风格单头文件 C89 库，用于构建高性能文本编辑器 buffer：
...
两库相互独立、可自由组合：piecetab 只存字节（"clean octet"——不管行、
不管编码），linecache 只管行断点。二者组合即得支持 O(log n) 偏移 ↔ 行号
双向导航的完整编辑器 buffer。
...
两库皆为 stb 风格：任意处包含头文件，在恰好一个编译单元中定义
`*_IMPLEMENTATION` 宏。
...
完整 API 参考见 [`docs/piecetab.zh.md`](docs/piecetab.zh.md) 与
...
构建。两个头文件均保持 **100% 行/函数覆盖**与约 90% 分支覆盖。
