...
static int ud_open_frag_n(bench_pt_ud *u, const bench_Params *p, size_t n) {
    ...
    if (!bench_corpus_fragmented(&u->corpus, u->S, p->seed, n)) return 0;
...
static int ud_open_edited_n(bench_pt_ud *u, const bench_Params *p, size_t n) {
    u->S = pt_open(NULL, NULL);
    if (u->S == NULL) return 0;
    if (!bench_corpus_edited(&u->corpus, u->S, p->seed, n)) return 0;
    u->buf = (char *)malloc(BENCH_READ_LEN);
    u->bufcap = BENCH_READ_LEN;
    return u->buf != NULL;
}
...
static void teardown_nav(void *ud) { ud_close((bench_pt_ud *)ud); }
...
static long calls_scan(void *ud) { return ((bench_pt_ud *)ud)->calls; }
...
static int run_append(void *ud, long iters) {
    ...
    for (i = 0; i < iters; ++i) {
        ...
        if (pt_locate(&u->cur, pt_bytes(pt_buffer(&u->cur))) != PT_OK) return 0;
...
static int run_commit(void *ud, long iters) {
    ...
    for (i = 0; i < iters; ++i) {
        ...
        if (u->work) pt_release(u->work);
        u->work = b;
        if (i + 1 < iters && pt_seek(&u->cur, u->work, 0) != PT_OK) return 0;
...
static int run_replay(void *ud, long iters) {
    ...
    for (i = 0; i < iters; ++i) {
        ...
        if (op == 0) {
            ...
            size_t off = (size_t)bench_range(
                    &u->rng, 0, (long)pt_bytes(pt_buffer(&u->cur)) - 1);
            if (pt_locate(&u->cur, off) != PT_OK) return 0;
        } else if (op == 3) {
            if (pt_edit(&u->cur, 0, "\n", 1) != PT_OK) return 0;
        } else if (op == 4) {
...
static bench_Case bench_cases[] = {
        {"pt_from", "single_piece", 100000, 0, setup_single, run_from,
         teardown_nav, NULL, 0},
        {"pt_seek", "fragmented_10k", 100000, 0, setup_nav, run_seek,
         teardown_nav, NULL, 0},
        {"pt_seek", "fragmented_100k", 100000, 0, setup_nav_100k, run_seek,
         teardown_nav, NULL, 0},
        {"pt_seek", "fragmented_1m", 100000, 0, setup_nav_1m, run_seek,
         teardown_nav, NULL, 0},
        {"pt_locate", "fragmented_10k", 100000, 0, setup_nav, run_locate,
         teardown_nav, NULL, 0},
        {"pt_locate", "fragmented_100k", 100000, 0, setup_nav_100k, run_locate,
         teardown_nav, NULL, 0},
        {"pt_locate", "fragmented_1m", 100000, 0, setup_nav_1m, run_locate,
         teardown_nav, NULL, 0},
        {"pt_advance", "fragmented_10k", 100000, 0, setup_nav, run_advance,
         teardown_nav, NULL, 0},
        {"pt_advance", "fragmented_100k", 100000, 0, setup_nav_100k,
         run_advance, teardown_nav, NULL, 0},
        {"pt_advance", "fragmented_1m", 100000, 0, setup_nav_1m, run_advance,
         teardown_nav, NULL, 0},
        {"pt_read", "fragmented_10k", 100000, 0, setup_nav, run_read,
         teardown_nav, NULL, 0},
        {"pt_read", "fragmented_100k", 100000, 0, setup_nav_100k, run_read,
         teardown_nav, NULL, 0},
        {"pt_read", "fragmented_1m", 100000, 0, setup_nav_1m, run_read,
         teardown_nav, NULL, 0},
        {"pt_next", "fragmented_10k", 20, 0, setup_nav, run_next, teardown_nav,
         calls_scan, 0},
        {"pt_next", "fragmented_100k", 20, 0, setup_nav_100k, run_next,
         teardown_nav, calls_scan, 0},
        {"pt_next", "fragmented_1m", 20, 0, setup_nav_1m, run_next,
         teardown_nav, calls_scan, 0},
        {"pt_prev", "fragmented_10k", 20, 0, setup_nav, run_prev, teardown_nav,
         calls_scan, 0},
        {"pt_prev", "fragmented_100k", 20, 0, setup_nav_100k, run_prev,
         teardown_nav, calls_scan, 0},
        {"pt_prev", "fragmented_1m", 20, 0, setup_nav_1m, run_prev,
         teardown_nav, calls_scan, 0},
        {"pt_edit", "fragmented_10k", 20000, 1, setup_edit, run_edit,
         teardown_dirty, NULL, 0},
        {"pt_edit", "fragmented_100k", 20000, 1, setup_edit_100k, run_edit,
         teardown_dirty, NULL, 0},
        {"pt_edit", "fragmented_1m", 20000, 1, setup_edit_1m, run_edit,
         teardown_dirty, NULL, 0},
        {"pt_edit_commit", "fragmented_10k", 20000, 1, setup_edit_commit,
         run_edit_commit, teardown_dirty, NULL, 0},
        {"pt_edit_commit", "fragmented_100k", 20000, 1, setup_edit_commit_100k,
         run_edit_commit, teardown_dirty, NULL, 0},
        {"pt_edit_commit", "fragmented_1m", 20000, 1, setup_edit_commit_1m,
         run_edit_commit, teardown_dirty, NULL, 0},
        {"pt_insert", "fragmented_10k", 20000, 1, setup_edit, run_insert,
         teardown_dirty, NULL, 0},
        {"pt_insert", "fragmented_100k", 20000, 1, setup_edit_100k, run_insert,
         teardown_dirty, NULL, 0},
        {"pt_insert", "fragmented_1m", 20000, 1, setup_edit_1m, run_insert,
         teardown_dirty, NULL, 0},
        {"pt_append", "fragmented_10k", 20000, 1, setup_edit, run_append,
         teardown_dirty, NULL, 0},
        {"pt_splice", "fragmented_10k", 20000, 1, setup_edit, run_splice,
         teardown_dirty, NULL, 0},
        {"pt_splice", "fragmented_100k", 20000, 1, setup_edit_100k, run_splice,
         teardown_dirty, NULL, 0},
        {"pt_splice", "fragmented_1m", 20000, 1, setup_edit_1m, run_splice,
         teardown_dirty, NULL, 0},
        {"pt_remove", "fragmented_10k", 20000, 1, setup_edit, run_remove,
         teardown_dirty, NULL, 0},
        {"pt_remove", "fragmented_100k", 20000, 1, setup_edit_100k, run_remove,
         teardown_dirty, NULL, 0},
        {"pt_remove", "fragmented_1m", 20000, 1, setup_edit_1m, run_remove,
         teardown_dirty, NULL, 0},
        {"pt_commit", "fragmented_10k", 1000, 1, setup_dirty, run_commit,
         teardown_dirty, NULL, 0},
        {"pt_commit", "fragmented_100k", 1000, 1, setup_dirty_100k, run_commit,
         teardown_dirty, NULL, 0},
        {"pt_commit", "fragmented_1m", 1000, 1, setup_dirty_1m, run_commit,
         teardown_dirty, NULL, 0},
        {"pt_rollback", "fragmented_10k", 1000, 1, setup_dirty, run_rollback,
         teardown_dirty, NULL, 0},
        {"pt_rollback", "fragmented_100k", 1000, 1, setup_dirty_100k,
         run_rollback, teardown_dirty, NULL, 0},
        {"pt_rollback", "fragmented_1m", 1000, 1, setup_dirty_1m, run_rollback,
         teardown_dirty, NULL, 0},
        {"pt_compact", "edited_10k", 20, 1, setup_compact, run_compact,
         teardown_nav, NULL, 0},
        {"pt_compact", "edited_100k", 20, 1, setup_compact_100k, run_compact,
         teardown_nav, NULL, 0},
        {"pt_compact", "edited_1m", 20, 1, setup_compact_1m, run_compact,
         teardown_nav, NULL, 0},
        {"pt_replay", "editor_script", 20000, 1, setup_replay, run_replay,
         teardown_dirty, NULL, 0},
};
...
int main(int argc, char **argv) {
    ...
    ok = bench_run(
            bench_cases, (int)(sizeof(bench_cases) / sizeof(bench_cases[0])),
            &params, out);
