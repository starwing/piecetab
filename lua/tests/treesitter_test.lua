...
function TestLang:testRequireC()
    ...
    lu.assertNotIsNil(lang)
    lu.assertIsTrue(lang.symbol_count > 0)
    lu.assertIsTrue(lang.version > 0)
end
...
function TestLang:testSymbolRoundtrip()
    ...
    lu.assertNotIsNil(lang:symbol(id))
end
...
function TestParse:testParseString()
    ...
    lu.assertIsTrue(t.root.end_byte > 0)
end
...
function TestParse:testReadFunctionInput()
    ...
    local t = assert(p:parse(nil, function(byte, row, col)
        lu.assertIsTrue(byte >= 0)
        lu.assertIsTrue(row >= 0)
        lu.assertIsTrue(col >= 0)
        return src:sub(byte)
    end))
    lu.assertNotIsNil(t)
    lu.assertEquals(t.root.type, "translation_unit")
...
function TestQuery:testCaptureMatch()
    ...
    lu.assertNotIsNil(q)
    lu.assertIsTrue(q.capture_count > 0)
...
function TestNodeFields:testChildAccess()
    ...
    lu.assertIsTrue(fd.child_count >= fd.named_child_count)
    lu.assertIsNil(t.root[99])
    lu.assertIsNil(t.root[0])
    ...
    lu.assertIsNil(fd:child("no_such_field"))
    local fid = C:field("declarator")
    lu.assertIsTrue(fid > 0)
...
function TestNodeFields:testNavigation()
    ...
    lu.assertIsNil(t.root.parent) -- root of tree: null node
    lu.assertIsNil(fd.next_sibling)
    lu.assertIsNil(fd.prev_sibling)
    lu.assertIsNil(fd.next_named_sibling)
    lu.assertIsNil(fd.prev_named_sibling)
    ...
    lu.assertIsNil(body.next_sibling)
    lu.assertIsNil(body.next_named_sibling)
    local d = assert(fd:first_child_for_byte(1))
    lu.assertEquals(d.type, "primitive_type")
    lu.assertIsTrue(fd:first_child_for_byte(20).type:match("compound") ~= nil)
    lu.assertIsNil(fd:first_child_for_byte(999))
    local d2 = assert(fd:first_named_child_for_byte(1))
    lu.assertEquals(d2.type, "primitive_type")
    lu.assertIsNil(fd:first_named_child_for_byte(999))
    lu.assertNotIsNil(fd:descendant_for_byte_range(5, 10))
    lu.assertNotIsNil(fd:descendant_for_byte_range(100, 200))
    lu.assertNotIsNil(fd:named_descendant_for_byte_range(1, 14))
    lu.assertNotIsNil(fd:named_descendant_for_byte_range(1, 2))
    lu.assertNotIsNil(fd:descendant_for_point_range(1, 1, 1, 14))
    lu.assertNotIsNil(fd:named_descendant_for_point_range(1, 5, 1, 9))
end

function TestNodeFields:testPredicates()
    local _, t = croot()
    local fd = assert(t.root[1])
    lu.assertIsFalse(fd.is_null)
    lu.assertIsTrue(fd.is_named)
    lu.assertIsFalse(fd.is_missing)
    lu.assertIsFalse(fd.is_extra)
    lu.assertIsFalse(fd.has_error)
    lu.assertIsFalse(fd.is_error)
    lu.assertIsFalse(fd.has_changes)
    lu.assertIsTrue(fd.parse_state > 0)
    lu.assertIsTrue(fd.next_parse_state > 0)
    lu.assertIsTrue(fd.descendant_count > 0)
end
...
function TestNodeFields:testIdentity()
    ...
    lu.assertIsTrue(fd.end_byte > fd.start_byte)
    local sr, sc = fd:start_point()
    local er, ec = fd:end_point()
    lu.assertIsTrue(sr >= 0 and sc >= 0 and er >= 0 and ec >= 0)
    lu.assertNotIsNil(fd.tree)
    lu.assertNotIsNil(fd.language)
    local _, t2 = croot()
...
function TestNodeFields:testMissingAndEdit()
    ...
    lu.assertIsTrue(t.root.has_error)
    local err = assert(t.root[1])
    lu.assertEquals(err.type, "ERROR")
    lu.assertIsTrue(err.is_error)
...
function TestTree:testIncludedRanges()
    ...
    lu.assertEquals(t.root.type, "translation_unit")
    lu.assertNotIsNil(t.included_ranges) -- tree reports the parse ranges
...
function TestParser:testReset()
    ...
    lu.assertNotIsNil(p:parse(nil, "int main(void) { return 0; }\n"))
    p:delete()
end
...
function TestParser:testLoggerError()
    ...
    lu.assertNotIsNil(p:parse(nil, CSRC))
    p.logger = nil
    p:delete()
end
...
function TestTreeCursor:testWalk()
    ...
    lu.assertIsTrue(c.depth >= 0)
    lu.assertIsTrue(c.descendant_index >= 0)
    lu.assertNotIsNil(c.field)   -- field id of current position
    lu.assertIsNil(c.field_name) -- root position has no field
...
function TestQueryMeta:testPatterns()
    ...
    lu.assertIsTrue(q.pattern_count >= 2)
    lu.assertIsTrue(q.capture_count >= 2)
    lu.assertIsTrue(q.string_count >= 0)
    lu.assertIsTrue(q:start_byte_for_pattern(1) >= 0)
    lu.assertIsTrue(q:is_pattern_rooted(1) == true)
    lu.assertIsTrue(q:is_pattern_non_local(1) == false)
    lu.assertIsTrue(q:is_pattern_guaranteed_at_step(1) == false)
...
function TestQueryMeta:testPredicates()
    ...
    lu.assertIsTrue(#preds >= 1)
    local steps = {}
    for _, t1, v1 in preds do steps[#steps + 1] = { t1, v1 } end
    lu.assertIsTrue(#steps >= 1)
    lu.assertNotIsNil(steps[1][1])
    lu.assertNotIsNil(steps[1][2])
...
function TestQueryMeta:testDisable()
    ...
    lu.assertNotIsNil(c:next_match())
    lu.assertEquals(#c, 0)
    q:disable_pattern(1)
    local count1 = matchn(q)
    lu.assertIsTrue(count1 < count0)
    q:delete()
end
...
function TestQueryCursor:testMatch()
    ...
    lu.assertNotIsNil(c:next_match())
    lu.assertIsTrue(#c >= 1)
    lu.assertNotIsNil(c[1])
    lu.assertNotIsNil(c[1].type)
    lu.assertIsNil(c[99])
    lu.assertIsNil(c[0])
    lu.assertIsTrue(c.match_id >= 0)
    lu.assertIsTrue(c.pattern_index >= 1)
    lu.assertIsFalse(c.did_exceed_match_limit)
    local id = c:next_capture()
    lu.assertIsTrue(id >= 1)
    local n, idx = c:captures(1)
    lu.assertNotIsNil(n)
    lu.assertIsTrue(idx >= 0)
    lu.assertIsNil(c:next_capture()) -- exhausted
    while c:next_match() do end
    lu.assertIsNil(c:next_match())
    c:delete()
end
...
function TestQueryCursor:testRanges()
    ...
    c2:exec(q, t.root)
    lu.assertNotIsNil(c2:next_match())
    c2:delete()
end
...
function TestQueryCursor:testLimits()
    ...
    local _, t = croot()
    local c = q:exec(t.root)
    lu.assertIsTrue(c.match_limit >= 0)
    c.match_limit = 1
...
function TestLanguage:testField()
    lu.assertIsTrue(C.field_count > 0)
    lu.assertIsTrue(C.state_count > 0)
    local fid = C:field("declarator")
    lu.assertIsTrue(fid > 0)
    ...
    lu.assertIsTrue(C:next_state(1, 1) >= 0)
...
function TestLookahead:testWalk()
    ...
    lu.assertNotIsNil(it.language)
    lu.assertIsTrue(it.symbol > 0)
    lu.assertNotIsNil(it.symbol_name)
    local seen = 0
    while it:next() do
        seen = seen + 1
        lu.assertIsTrue(it.symbol > 0)
    end
    lu.assertIsTrue(seen > 0)
    lu.assertIsTrue(it:reset(1))
    lu.assertIsTrue(it:reset(C, 1))
...
function TestSlice:testChangedRanges()
    ...
    lu.assertIsTrue(#r >= 1)
    lu.assertIsTrue(r[1] > 0)        -- single-value index: start_byte
    local fields
    for _, a, b, c1, d, e, f in r do -- full 6 fields via iteration
        fields = { a, b, c1, d, e, f }
    end
    lu.assertNotIsNil(fields)
    lu.assertIsTrue(fields[2] >= fields[1])
    lu.assertIsNil(r[99])
    ---@diagnostic disable-next-line: undefined-field -- no metatable auto-call
    lu.assertIsNil(r.no_such_field)
    lu.assertNotIsNil(tostring(r))
    local n = 0
...
function TestErrors:testParseErrors()
    ...
    lu.assertNotIsNil(r1.root)
    local r2 = assert(p:parse(nil, function() return 42 end))
    lu.assertNotIsNil(r2.root)
    p:delete()
...
function TestErrors:testRequireVariants()
    ...
    lu.assertNotIsNil(ts.require("c", "tree_sitter_c")) -- explicit symbol
