...
static size_t utf8_encode(char *buff, utfint x) {
    ...
    if (x < 0x80) /* ascii? */
        ...
        buff[UTF8_BUFFSZ - n] = (char)(((~mfb << 1) | x) & 0xFF); /* first */
    }
    return n;
}
...
static const char *utf8_decode(const char *s, utfint *val, int strict) {
    ...
    if (strict && (res > UTF8_MAXCP || (0xD800u <= res && res <= 0xDFFFu)))
        return NULL;
    if (val) *val = res;
...
static const char *utf8_relat(const char *s, const char *e, int idx) {
    if (idx >= 0) return utf8_offset(s, e, 1, idx - 1);
...
static const char *utf8_invalid_offset(const char *s, const char *e) {
    while (s < e) {
        uint8_t c = *s;
        if (c >= 0x80) {
            /* c < 0xC0 means a continuation byte, but we are not in the
             *          middle of a multi-byte code unit
             * c >= 0xC0 && c < 0xC2 means an overlong 2-byte code unit
             * c >= 0xF8 means a 5-byte or 6-byte code unit, which is illegal,
             *           or else illegal byte 0xFE/0xFF
             * c >= 0xF5 && c < 0xF8 means a 4-byte code unit encoding
            ...
            if (needed_bytes >= 3) {
                uint8_t c3 = s[2];
                if ((c3 & 0xC0) != 0x80)
                    return s; /* 3rd byte of code is not a continuation byte */
                if (needed_bytes == 3) {
                    if (c == 0xE0 && c2 < 0xA0)
...
static int nfc_check(
    ...
    if (reason == REASON_MUST_CONVERT_1 || reason == REASON_MUST_CONVERT_2) {
        /* This codepoint has a different, canonical form, so this string is not
         * NFC */
        return 0;
    } else if (reason == REASON_STARTER_CAN_COMBINE) {
        /* It is possible that this 'starter' codepoint should have been
         * combined with the preceding 'starter' codepoint; if so, this string
         * is not NFC */
        ...
        /* Combining mark; check if it should have been combined with preceding
         * starter codepoint */
        ...
        /* Could it be that preceding 'starter' codepoint is already combined,
         * but with a combining mark which is out of order with this one? */
        ...
        if (!prev_canon_cls && starter >= 0x1100 && starter <= 0x1112) {
            /* Preceding codepoint was a leading jamo; they should have been
             * combined */
            return 0;
        }
    } else if (reason == REASON_JAMO_TRAILING) {
        if (!prev_canon_cls && starter >= 0xAC00 && starter <= 0xD7A3) {
            /* Preceding codepoint was a precomposed Hangul syllable; check if
             * it had no trailing jamo */
            if ((starter - 0xAC00) % 28 == 0) {
                /* It didn't have a trailing jamo, so this trailing jamo should
                 * have been combined */
                return 0;
            }
        }
...
static void stable_sort_combining_marks(
    ...
    /* We need to use a stable sort for sorting combining marks which are in the
     * wrong order when doing NFC normalization; bottom-up merge sort is fast
     * and stable */
    size_t       limit = size - 1;
    unsigned int i;
    for (i = 0; i < limit; i += 2) {
    ...
    while (runsize < size) {
        size_t i, blocksize = runsize
                            * 2; /* We will now sort slices of this size */
        ...
        if (size - limit > runsize) {
            ...
            memcpy(&dest[limit], &src[limit],
                   (size - limit) * sizeof(uint32_t));
        }
        /* After each series of (progressively larger) merges, we swap src &
         * dest to avoid memcpy'ing the partially sorted results from dest back
         * into src */
        uint32_t *temp = src;
        src = dest;
        dest = temp;
...
static void grow_vector_if_needed(
        uint32_t **vector, uint32_t *onstack, size_t *size, size_t needed) {
    size_t current_size = *size;
    if (needed >= current_size) {
        size_t new_size = current_size * 2; /* `needed` is never bigger than
                                               `current_size * 2` */
...
static void string_to_nfc(
     ...
     * 1) Ensuring that codepoints with "built-in" accents are used whenever
     * possible rather than separate codepoints for a base character and
     * combining mark 2) Where combining marks must be used, putting them into
     * canonical order 3) Converting some deprecated codepoints to the
     * recommended variant 4) Ensuring that Korean Hangul are represented as
     * precomposed syllable codepoints whenever possible, rather than sequences
     * of Jamo codepoints
     *
     * (Combining marks are accents which appear on top of or below the
     * preceding character. Starter codepoints are the base characters which
     * combining marks can 'combine' with. Almost all codepoints are starters,
     * including all the Latin alphabet. Every Unicode codepoint has a numeric
     * 'canonicalization class'; starters have class = 0. Combining marks must
     * be sorted in order of their canonicalization class. Since the
     * canonicalization class numbers are not unique, the sort must be stable.)
     *
     * When converting to NFC, the largest scope which we need to work on at
     * once consists of a 'starter' codepoint and either 1 or more ensuing
     * combining marks, OR else a directly following starter codepoint.
     *
     * As we walk through the string, whenever we pass by a complete sequence of
     * starter + combining marks or starter + starter, we process that sequence
     * to see if it is NFC or not. If it is, we memcpy the bytes verbatim into
     * the output buffer. If it is not, then we convert the codepoints to NFC
     * and then emit those codepoints as UTF-8 bytes. */

    utfint      starter = (utfint)-1,
                ch;          /* 'starter' is last starter codepoint seen */
    const char *to_copy = s; /* pointer to next bytes we might need to memcpy
                                into output buffer */
    unsigned int prev_canon_cls = 0;
    int fixedup = 0; /* has the sequence currently under consideration been
                        modified to make it NFC? */

    /* Temporary storage for a sequence of consecutive combining marks
     * In the vast majority of cases, this small on-stack array will provide
     * enough space; if not, we will switch to a malloc'd buffer */
    uint32_t   onstack[8];
    ...
    while (s < e) {
        ...
        if (!canon_cls) {
            ...
            if (entry && entry->reason == REASON_MUST_CONVERT_2) {
                ...
                if (canon_cls1) {
                    ...
                    grow_vector_if_needed(
                            &vector, onstack, &vec_max, vec_size + 2);
            ...
            /* Handle preceding starter and optional sequence of combining marks
             * which may have followed it */
            if (prev_canon_cls) {
            ...
            process_combining_marks:
                ...
                while (i < vec_size) {
                    ...
                    if (mark_entry) {
                        if (mark_entry->reason == REASON_MUST_CONVERT_1) {
                            /* This combining mark must be converted to a
                             * different one */
                            vector[i] = (mark_entry->data1 << 8)
                                      | mark_entry->data2;
                            fixedup = 1;
                            continue;
                        } else if (
                                mark_entry->reason == REASON_MUST_CONVERT_2) {
                            /* This combining mark must be converted to two
                             * others */
                            grow_vector_if_needed(
                                    &vector, onstack, &vec_max, vec_size + 1);
                            memmove(&vector[i + 2], &vector[i + 1],
                                    sizeof(uint32_t) * (vec_size - i - 1));
                            vector[i] = (mark_entry->data1 << 8)
                                      | lookup_canon_cls(mark_entry->data1);
                            vector[i + 1] = (mark_entry->data2 << 8)
                                          | lookup_canon_cls(mark_entry->data2);
                            vec_size++;
                            fixedup = 1;
                            continue;
                        } else if (
                                mark_entry->reason == REASON_COMBINING_MARK) {
                            unsigned int mark_canon_cls = vector[i] & 0xFF;
                            if (i == 0
                                || mark_canon_cls > (vector[i - 1] & 0xFF)) {
                                if (nfc_combine(
                                            starter, combine_mark, &starter)) {
                                    /* This combining mark must be combined with
                                     * preceding starter */
                                    vec_size--;
                                    memmove(&vector[i], &vector[i + 1],
                                            sizeof(uint32_t)
                                                    * (vec_size
                                                       - i)); /* Remove element
                                                                 i */
                                ...
                                decompose_table *decomp = nfc_decompose(
                                        starter);
                                if (decomp) {
                                    if (decomp->canon_cls2 > mark_canon_cls
                                        && nfc_combine(
                                                decomp->to1, combine_mark,
                                                &starter)) {
                                        /* The preceding starter already
                                         * included an accent, but when
                                         * represented as a combining mark, that
                                         * accent has a HIGHER canonicalization
                                         * class than this one Further, this one
                                         * is able to combine with the same base
                                         * character In other words, the base
                                         * character was wrongly combined with a
                                         * "lower-priority" combining mark; fix
                                         * that up */
                                        unsigned int class2 = lookup_canon_cls(
                                                decomp->to2);
                                        memmove(&vector[1], &vector[0],
                                                sizeof(uint32_t) * i);
                                        vector[0] = (decomp->to2 << 8) | class2;
                                        stable_insert_combining_mark(
                                                vector, vec_size, 0);
                                        fixedup = 1;
                                        continue;
                                    } else {
                                        decompose_table
                                                *decomp2 = nfc_decompose(
                                                        decomp->to1);
                                        if (decomp2
                                            && decomp2->canon_cls2
                                                       > mark_canon_cls
                                            && nfc_combine(
                                                    decomp2->to1, combine_mark,
                                                    &starter)) {
                                            grow_vector_if_needed(
                                                    &vector, onstack, &vec_max,
                                                    vec_size + 1);
                                            memmove(&vector[i + 2],
                                                    &vector[i + 1],
                                                    sizeof(uint32_t)
                                                            * (vec_size - i
                                                               - 1));
                                            memmove(&vector[2], &vector[0],
                                                    sizeof(uint32_t) * i);
                                            vector[0] = (decomp2->to2 << 8)
                                                      | lookup_canon_cls(
                                                                decomp2->to2);
                                            vector[1] = (decomp->to2 << 8)
                                                      | lookup_canon_cls(
                                                                decomp->to2);
                                            vec_size++;
                                            stable_insert_combining_mark(
                ...
                if (fixedup) {
                    unsigned int i;
                    /* The preceding starter/combining mark sequence was bad;
                     * convert fixed-up codepoints to UTF-8 bytes */
                    if (starter != (utfint)-1) add_utf8char(buff, starter);
                    for (i = 0; i < vec_size; i++)
                        add_utf8char(buff, vector[i] >> 8);
                } else {
                    /* The preceding starter/combining mark sequence was good;
                     * copy raw bytes to output */
                    luaL_addlstring(buff, to_copy, s - to_copy);
                }
                if (s >= e) {
                    /* We jumped in to the middle of the main loop to finish
                     * processing trailing combining marks... we are actually
                     * done now */
                    if (vector != onstack) free(vector);
                    return;
                }
                vec_size = 0; /* Clear vector of combining marks in readiness
                                 for next such sequence */
                fixedup = 0;
                ...
                fixedup = 0;
                if (entry) {
                    if (entry->reason == REASON_STARTER_CAN_COMBINE
                        && nfc_combine(starter, ch, &ch)) {
                        fixedup = 1;
                    } else if (
                            entry->reason == REASON_JAMO_VOWEL
                            && starter >= 0x1100 && starter <= 0x1112) {
                        ch = 0xAC00 + ((starter - 0x1100) * 588)
                           + ((ch - 0x1161) * 28);
                        fixedup = 1;
                ...
                if (!fixedup)
                    add_utf8char(
                            buff,
                            starter); /* Emit previous starter to output */
            }
            starter = ch;
            to_copy = s;

            /* We are finished processing the preceding starter and optional
             * sequence of combining marks Now check if this (possibly
             * deprecated) starter needs to be converted to a canonical variant
             */
            if (entry) {
                if (entry->reason == REASON_MUST_CONVERT_1) {
                    ...
                    utfint conv1 = entry->data1;
                    utfint conv2 = entry->data2;
                    /* It's possible that 'ch' might convert to two other
                     * codepoints, where the 2nd one is a combining mark */
                    unsigned int canon_cls2 = lookup_canon_cls(conv2);
                    if (canon_cls2) {
                        /* It's possible that the 1st resulting codepoint may
                         * need to be split again into more codepoints */
                        nfc_table *conv_entry = nfc_quickcheck(conv1);
                        if (conv_entry
                            && conv_entry->reason == REASON_MUST_CONVERT_2) {
                            utfint       conv3 = conv2;
    ...
    if (vec_size)
        goto process_combining_marks; /* Finish processing trailing combining
                                         marks */
    if (starter != (utfint)-1) add_utf8char(buff, starter);
...
utf8_categories(define_category) utf8_converters(define_converter)
...
#undef bind_converter
        ...
        static const char *parse_escape(
                lua_State *L, const char *s, const char *e, int hex,
                utfint *pch) {
    utfint code = 0;
    int    in_bracket = 0;
    if (*s == '{') ++s, in_bracket = 1;
...
static int Lutf8_escape(lua_State *L) {
    ...
    while (s < e) {
        ...
        if (ch == '%') {
            ...
            default: s = utf8_safe_decode(L, s, &ch); goto next;
            }
...
static int Lutf8_width(lua_State *L) {
    ...
    return lua_pushinteger(
                   L, utf8_width(
                              CAST(utfint, lua_tointeger(L, 1)), ambiwidth,
                              default_width)),
           1;
}
    ...
    if (p != ms->p_end) { /* end of pattern? */
        ...
        case L_ESC: { /* escaped sequence not in the format class[*+?-]? */
            ...
            case 'b': { /* balanced string? */
                ...
                if (s != NULL) goto init; /* return match(ms, s, p + 4); */
                /* else fail (s == NULL) */
                break;
            }
            case 'f': { /* frontier? */
                ...
                if (*p != '[')
                    luaL_error(
                            ms->L, "missing " LUA_QL("[") " after " LUA_QL(
                                           "%%f") " in pattern");
        ...
        do {
            ...
            if ((res = match(&ms, init, p)) != NULL) {
                if (find) {
                    ...
                    lua_pushinteger(
                            L, idx + utf8_length(init, res) - 1); /* end */
...
static int Lutf8_isnfc(lua_State *L) {
    ...
    while (s < e) {
        ...
        if (entry
            && !nfc_check(ch, entry, starter, canon_cls, prev_canon_cls)) {
            lua_pushboolean(L, 0); /* Return false */
            return 1;
        }
...
static int Lutf8_normalize_nfc(lua_State *L) {
    ...
    while (p < e) {
        ...
        unsigned int canon_cls = lookup_canon_cls(ch);
        if (canon_cls && canon_cls < prev_canon_cls) {
            goto build_string; /* Combining marks are out of order; this string
                                  is not NFC */
        }
    ...
    lua_settop(L, 1);      /* Return input string without modification */
    lua_pushboolean(L, 1); /* String was in normal form already, so 2nd return
                              value is 'true' */
    return 2;
    ...
    while (1) {
        ...
        if (ch == '\r') {
            ...
            break;
                ...
                find_in_range(cntrl_table, table_size(cntrl_table), ch)
                && !find_in_range(prepend_table, table_size(prepend_table), ch)
                && ch != 0x200D
                && !find_in_range(
                        compose_table, table_size(compose_table), ch)) {
            /* Control characters do not bind to anything, except when they are
             * also Grapheme_Extend */
            break;
            ...
            if (next_p < e
                && find_in_range(
                        ...
                        ch)) {
                /* After an Extended_Pictographic codepoint and ZWJ, we bind to
                 * a following Extended_Pictographic */
                utfint      nextnext_ch;
                const char *probe_ep = utf8_safe_decode(
                        L, next_p, &nextnext_ch);
                if (find_in_range(
                            pictographic_table, table_size(pictographic_table),
                            nextnext_ch)) {
                    p = probe_ep;
            ...
            bind = 1;
                ...
                find_in_range(cntrl_table, table_size(cntrl_table), next_ch)
                && !find_in_range(
                        prepend_table, table_size(prepend_table), next_ch)
                && !find_in_range(
                        compose_table, table_size(compose_table), next_ch)) {
            /* Control characters do not bind to anything, except when they are
             * also Grapheme_Extend */
            break;
        } else {
            if (indic_conjunct_type(ch) == INDIC_CONSONANT) {
                ...
                int         saw_linker = 0;
                while (indic_type) {
                    /* Consume any number of Extend or Linker codepoints,
                     * followed by a single Consonant The sequence must contain
                     * at least one Linker, however! */
            ...
            if (find_in_range(compose_table, table_size(compose_table), next_ch)
                || (next_ch >= 0x1F3FB && next_ch <= 0x1F3FF)) {
                /* The 2nd codepoint has property Grapheme_Extend, or is an
                 * Emoji_Modifier codepoint */
                if (next_p < e
                    && find_in_range(
                            pictographic_table, table_size(pictographic_table),
                            ch)) {
                    /* Consume any number of 'extend' codepoints, one ZWJ, and
                    ...
                    while (probe < e) {
                        ...
                        if (probed_ch == 0x200D) {
                            ...
                            break;
                                ...
                                find_in_range(
                                        compose_table,
                                        table_size(compose_table), probed_ch)
                                || (probed_ch >= 0x1F3FB
                                    && probed_ch <= 0x1F3FF)) {
                            next_p = probe;
                            next_ch = probed_ch;
                ...
                bind = 1;
                    ...
                    find_in_range(
                            prepend_table, table_size(prepend_table), ch)) {
                /* The 1st codepoint has property Prepend_Concatenation_Mark, or
                 * is a type of Indic Syllable which binds to the following
                 * codepoint */
                bind = 1;
            } else if (
                    ch >= 0x1F1E6 && ch <= 0x1F1FF && next_ch >= 0x1F1E6
                    && next_ch <= 0x1F1FF) {
                /* Regional Indicator (flag) emoji bind together; but only in
                 * twos */
                p = next_p;
                ch = 0xFFFE; /* Set 'ch' to bogus value so we will not re-enter
                                this branch on next iteration */
                continue;
            } else {
                /* Korean Hangul codepoints have their own special rules about
                 * when they are considered a single grapheme cluster */
                    ...
                    if (hangul2) {
                        if (hangul1 == HANGUL_L) {
                            bind = (hangul2 != HANGUL_T);
                                ...
                                hangul1 == HANGUL_LV || hangul1 == HANGUL_V) {
                            bind = (hangul2 == HANGUL_V || hangul2 == HANGUL_T);
                        } else if (
                                hangul1 == HANGUL_LVT || hangul1 == HANGUL_T) {
                            bind = (hangul2 == HANGUL_T);
                        }
                    }
                }
