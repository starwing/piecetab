...
**lc_append** 在游标位置裂开树，将右侧数据搬入临时数组 `rt[]`，以 append 消化 scanner 输出并通过 fillrt 扩容，最后将 rt 缝合回树并补写裂点残字节。
...
| `lc_append`       | 入口   | 裂叶→append+fillrt→stitch→fixremain          |
...
1. **append 后 paths[levels] 指向空位**：返回时 `paths[levels]` 指向下一待分配叶槽位（非最后有效叶）。lc_scan 若 `C.off != 0` 则后撤一位；lc_append 的 fillrt 从此空位起建空链扩容。append 是 `off/nu/loff/lnu` 唯一增量更新者。
