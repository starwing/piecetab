# lc_insert 与 lc_splice 算法设计 v2

## 1. 概述

本文档描述 linecache 的中部插入 (`lc_insert`) 与区间删除 (`lc_splice`) 算法。二者共用一套 B+ 树操作原语（裂叶、缝合、折叠、均分、缩根），但因语义不同（插入是"裂开→追加→缝合"，删除是"修剪→挖掘→缝合→修复"），流程各异。

**lc_insert** 在游标位置裂开树，将右侧数据搬入临时数组 `rt[]`，以 append 消化 scanner 输出并通过 fillrt 扩容，最后将 rt 缝合回树并补写裂点残字节。
...
| 术语 | 含义 |
|------|------|
| `rt[]` | 临时节点数组，mediates fillrt 与 stitch 之间的数据搬运。`rt[k]` 对应树中第 `levels - k` 层 |
...
| fillrt | 向上寻首个非末位层，搬右兄弟入 `rt`，垂空链至目标层。`addroom` 的别名 |
...
| `lcD_findroom` | 扩容 | 向上寻非末位层 + 搬右兄弟 + 垂空链（fillrt） |
...
| `lc_insert` | 入口 | 裂叶→append+fillrt→stitch→fixremain |
...
- 返回值 = 1 表示父满且末叶满（需 fillrt 扩容）；= 0 表示 scanner 耗尽；< 0 表示 OOM
...
- **paths 指向空位而非有效叶**：调用方（lc_scan 或 lc_insert）自行决定是否需要回退
...
向上寻首个非末位层 `fl`，搬右兄弟入 `rt`，垂空链至 `l` 层。stitchnode 与 lc_insert 共用。
...
- **找 fl**（自 `fl = l - 1` 向上至 `0`）：
  ...
  - 为何不直接用 `child_count`？因 C 在末位、但父节点未必满——仅需右侧有槽位即可停止上溯。fillrt 调用时 `l` 层父必满且 C 在末位，故 `fl == l` 时条件必假，循环至少下移一层
...
stitch 的主引擎。洋葱序（自叶向根，k=0..levels）将 `rt[k]` 中数据搬回树。边缝合边修复 underfill：先填满当前父节点（防溢出机制），再逐层 foldnode、最后 fillrt 为剩余数据建新节点链。

### 前置条件

- `rt[0..levels]` 存有待缝合数据（来自 cutleaf/fillrt/splicerange）
- `C->paths[0..levels]` 有效（每层在末位或末位后）
- `fillrt` 不会 OOM（stitch 入口已 `lcP_reserve` 预分配节点）
...
- **全搬完早退**：`if (!(m < rtcc)) continue`（虽进入修复块但 m == rtcc，无需 fillrt）
- **次复制**（`m < rtcc`，首复制未搬尽）：
  - `l = kl`：待修复层级下界降至当前层
  - `d = k ? LC_FANOUT - lcK_idx(C, lcK_parent(C, l), l) : m`
    - 叶层（k==0）：`d = m`——当前父节点中新搬入的子节点数
    - 内层（k>0）：`d = LC_FANOUT - idx(C, parent(l), l)`——当前父节点中 C 右侧的槽位数（fillrt 前父节点必满，此值为右侧搬出子节点数）
...
为何延迟？fillrt 搬右兄弟入 rt 时，C 所在路径的**左侧**节点必然已满（若不满，fillrt 的上溯循环会停在更低的层）。而 C 路径上唯一可能 underfill 的节点在 fillrt 新垂链的**右侧**单链树上。因此需先在前轮 foldnode 修复右侧单链树的 underfill，再通过次轮 backwardnode 修正 C 的 paths 指向。
...
lc_insert 之缝合入口。调度 mergeleaf → stitchnode → foldleaf → 位置修正。

### 前置条件

- `lcP_reserve` 已预分配 `l + 1` 个节点（保 fillrt 不 OOM）
...
OOM 时恢复树至 cutleaf 后状态。降根至裂叶时高度（清除 append/fillrt 新增的 scanner 数据）、合并裂点叶、释右侧孤子树、缝合 rt。返回 `LC_ERRMEM`。

### 前置条件

- `sC` 为 cutleaf 后的游标快照，`sl = lcK_levels(sC)`
- 树已部分修改（append/fillrt 可能已写入部分数据、可能已 rootpush 增层）

### 行为

- **降根**（`for (k = levels; k > sl; --k)`）：清除 fillrt rootpush 增加的层
...
- **L 侧**：`i = idx(L, p, kl)`。L 右侧的兄弟子树全部删除：汇总度量 → `lcM_up` 扣祖先 → `lcN_erase` 释放并压实父节点
...
- L 侧：`i = idx(L, p, l)`。汇总 L 与 R 之间（i+1..cc）的度量 → `lcM_up` 扣祖先 → `lcN_erase` 释放中段子树并压实。中段子树直接删除，不存于 rt
...
1. **append 后 paths[levels] 指向空位**：返回时 `paths[levels]` 指向下一待分配叶槽位（非最后有效叶）。lc_scan 若 `C.off != 0` 则后撤一位；lc_insert 的 fillrt 从此空位起建空链扩容。append 是 `off/nu/loff/lnu` 唯一增量更新者。

2. **stitchnode 中 C 总在可能 underfill 的节点上**：fillrt 的垂链策略保证 C->paths 路径上的节点左侧必为满节点（若不满，fillrt 上溯会停在更低层），唯一可能 underfill 者就是 C->paths 所在的右侧单链。故 foldnode 时 `left=1` 优先左探，自动处理 underfill。

3. **d 的延迟生效**：stitchnode 的 `d` 在后一轮迭代中经 backwardnode 生效。fillrt 建新链时右侧单链节点未及修复，此轮 d 记录的是"待修复的右侧子节点数"。前轮 foldnode 修复右链 underfill，后轮 backwardnode 用 d 修正 C->paths。
