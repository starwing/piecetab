...
1. 分配新 `pt_Tree`：`root = copy_of_blob_root`，`version = ++S->max_version`，
   `from = 旧 b` 并 `retain(from)`（COW 生命周期：保证源树的共享节点存活到
   本 transient 释放，见 §4.4）
2. `C->tree = 新 tree`，`C->dirty = 1`

> **注**：不再用 `C->oldblob` 记 rollback 锚点——改由 `b->from` 表达
...
**使用者负责持有"要回退到的 blob"**——持有则 rollback 后可继续用，放弃则
cursor 失效。
...
**问题**：blob 上存在多个 Cursor（C1 派生出 C2/C3…）时，某些操作会让其他
Cursor 静默失效，而库无法检测：
...
pt_Blob    pt_empty(pt_State *S);
pt_Blob    pt_from(pt_State *S, const char *s, size_t len);
int        pt_retain(pt_Blob b);
int        pt_release(pt_Blob b);
pt_Size    pt_bytes(pt_Blob b);
int        pt_version(pt_Blob b);
```
...
```c
/* construction */
int  pt_seek(pt_Cursor *C, pt_Blob b, size_t off); /* Cursor 构造器: reset + bind + 定位, 清 dirty */
...
- `pt_seek` 是 Cursor 构造器，重置所有内部状态（含 dirty），适合首次绑定 blobs
...
| persistent | 持久态 tree（immutable blob，可 COW 共享）                            |
