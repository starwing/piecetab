...
支持 COW 快照（Blob）、可变编辑态（hole 叶）、事务 commit/rollback。
...
### pt_Tree — 树（Blob 即 `const pt_Tree *`）
...
| Blob     | `pt_empty`, `pt_from`, `pt_retain`, `pt_release` | 构造/引用计数。`pt_empty` 返回哨兵零分配       |
...
- **commit**: `ptC_holebytes` 汇总 → `pt_reserve` arena → `ptC_freeze` DFS
  ...
  换为 literal 指针 → 清 dirty 返回 Blob
- **rollback**: `from->refc == 1` 则源树必死 → cursor 失效（tree=NULL）；
  否则回到 from。契约：使用者负责持有要回退到的 blob（design v2 §4.4.1）
...
| `notes/design_generational_arena.md` / `impl_generational_arena.md` | arena 设计与实现 |
| `notes/plan_piecetab_v2_edit.md`  | pt_edit 实现计划                        |
| `notes/plan_piecetab_v2_remove.md` / `impl_piecetab_v2_remove.md` | remove 实现 |
| `notes/remove_vs_linecache_diff.md` | remove 与 lc 删除算法差异             |
| `notes/design_marktree.md`        | 标记树（外置，未实现）                  |
