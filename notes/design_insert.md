...
- **lc_append = lcB_cutleaf ⊛ lcB_append ⊛ lcB_fillrt + lcB_stitch + lcB_fixremain**（裂开插入点后轮流追叶-搬右，stitch 缝合回树并整树，fixremain 补裂点残字节）
...
- **lc_append** 在中间插入，用 cutleaf 裂开插入点，fillrt 搬右兄弟入 `rt[]`，stitch 缝合回树并整树（含 balancerange fold），fixremain 补裂点残字节
...
**与 lc_append 之关系**：lc_append 固定走 cutleaf → append+fillrt → stitch + fixremain，与 lc_scan 独立。lc_scan 纯追加至树尾，不涉裂叶与缝合。
...
- **append 后 paths[levels] 指向空叶位**：返回时 `paths[levels]` 指向下一待分配叶槽位（非最后有效叶）。lc_scan 据此若 `C.off != 0` 则后撤一位指回有效叶；lc_append 的 fillrt 从此空位起建空链扩容。append 是全局 off/idx/loff/lidx 唯一增量更新者。

- **stitch 中，C->paths 始终在可能underfill的节点上**：所以如果遇到需要 `fillrt` 扩充单链树的情况，必定先填满前节点，以保证修复的时候 C->paths 所在的单链树节点是唯一可能 underfill 的节点

- **rm 与 e 须最后更新**：`rm`（裂点行前半残字节）与 e（尾缀字节）皆非完整行，仅存为 `lc_append` 局部变量，不可入树。必待全流程（S2-S4）成功后于 S5 补写——若提前入树而 append/fillrt/stitch 后续失败，rollback 无从撤销已写入树之字节，度量将永久错乱。
...
| lc_append        | 主流程（cutleaf ⊛ append+fillrt + stitch + fixremain） | 28   |
| **总计**         |                                                        | ~240 |
