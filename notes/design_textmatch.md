...
- 签名短，全部低于 80 列；
- source 侧的 cache/off/current/prev 集中在 `tm_State`；匹配链只传
...
static void   tmK_save(tm_Match *M, tm_Save *save);
static void   tmK_restore(tm_Match *M, const tm_Save *save);
static int    tmK_isboundary(tm_Match *M);
static int    tmK_islinestart(tm_Match *M);
static int    tmK_islineend(tm_Match *M);
```

底层函数（匹配层不可见，只接收 `tm_State *S`）：
...
- `p`：cache 内当前字节的挥发指针；`NULL` 表示未 materialize、位于 cache
  末尾、或 cache 已被更换。
- `tmK_peek`：`current` 已知直接返回；`p == NULL` 时先建立 `p`（必要时
  load），然后解码当前字符并设置 `next`。
- `tmK_next`：`prev = current`，前进到下一 codepoint 并解码；前进后用
  `tmK_setp` 重新 materialize `p`。
- `tmK_prev`：回退到前一个 codepoint 起点并解码；`off == 0` 时返回 `0`，
...
- `tmK_save/restore`：保存/恢复 `off/next/current/prev`；restore 后用
  `tmK_setp` 重建 `p`。
- `tmK_setp`：根据当前 `off` 与 cache 的包含关系设置 `p`；`off` 在 cache
  内且不是末尾时指向对应字节，否则置 `NULL`。
- `tmK_isboundary/islinestart/islineend`：基于 `prev`/`peek` 的语义谓词，
  匹配层不再直接读“off 前面的字节”。

`next` 是 source 游标状态的一部分，任何保存/恢复游标的代码都必须同时
保存/恢复 `off`/`next`/`current`/`prev`。`p` 不进入 save/restore，因为它
可以由 `off` 与 cache 重建。

`tmR_load` 在真正更换 cache 时会把 `p` 置 `NULL`。`tmK_peek` 的 fill 分支
只处理 `p == NULL` 的情况；顺序前进时 `tmK_next` 已负责维护 `p`，因此
peek 的热路径不需要做 `tmR_incache` 范围判断。
`tmR_at` 只用于真正需要任意方向/任意 offset 的随机访问场合。
...
- 匹配失败回溯时通过 `tmK_save`/`tmK_restore` 保存/恢复 `off/next/current/prev`
...
- `tm_Slice` 表示 pattern 中的一个片段（如 `[...]` 的括号体），
  传给 `tmM_bracketclass`、`tmM_single`、`tmM_maxexpand`、`tmM_minexpand`；
  匹配链中需要推进 pattern 时传 `tm_Slice *`。
- `tmL_len(s)` 返回 slice 字节数，`tmL_empty(s)` 判断空 slice（NULL 哨兵也视为空）；
  这两个小 helper 放在 implementation 前部，供所有 slice 操作复用。
- 内部匹配函数（`tmM_match`/`tmM_trymatch`/`tmM_maxexpand` 等）返回 `int` 状态码
  （`TM_MATCHED`/`TM_OK`/负错误码），成功位置通过 `tmK_offset(M)` 表达；
...
- 只读 pattern 范围传 `tm_Slice`；需要前进/返回解码位置时传 `tm_Slice *`
  （如 `tmU_decode`、`tmM_classend`、`tmM_balance`）。

### 关键匹配路径

- 单字符项：`tmK_peek` 取当前字符，匹配成功后 `tmK_next` 前进。
- `%b`：用 `tmK_next` 返回值逐字符数嵌套；闭合符命中且计数归零时再
  `tmK_next` 一次，把闭合符本身消费掉。
- `%f`：通过 `tmK_prevcp(M)` 取得前一个 codepoint，匹配层不直接碰 `prev`。
- `*` / `+` / `-`：`tmK_next` / `tmK_prev` 返回值通常丢弃，以 `tmK_offset(M)` /
  `tmK_peek(M)` 为真相；`?`/`*`/`-` 的零次回溯通过 `tmK_save`/`tmK_restore` 完成。
- Backrefs（`%1..%9`）通过 `tmM_backref(M, ch)` 完成；literal 搜索使用独立低层
  helper（`tmR_equal`、`tmR_equalmem`、`tmR_find`），这些 helper 只接收
...
- source 侧 cache/off/current/prev 集中在 `tm_State`；匹配链只传 `tm_Match *M`，不传 `tm_State *S`/`off`/`Cursor`
