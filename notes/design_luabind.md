...
- `seek("line", n)`：n ≤ breaks → `lc_seekline` + `lc_lineoffset` 返回行首；
  n > breaks → argcheck。
- `lines()` 迭代 i ∈ [0, breaks]：i < breaks → lc_seekline + lc_linelen
  读含 \n 完整行；i == breaks → 读 [lc_bytes, pt_bytes) 残段。
  迭代器持 doc 引用防 GC。
...
| 超长函数 | lpt_docsync / Ldoc_read / Ldoc_lineiter | 3 | 违反 25/30 行限 |

**结论**：当前代码违反胶水层铁律——做了大量 API 自身已提供的
边界处理（零参数 no-op、残段长度获取）。重写时应信任 API 语义。
