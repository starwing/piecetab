 ...
 * segments in a settled tree */

SP_STATIC int sp_checknode(const sp_Node *n, int rl, int mc) {
    size_t bsum;
    int    i;
    ...
    for (i = 0; i < (int)n->child_count; ++i) {
        if (rl == 0) {
            ...
            if (!sp_checknode(n->children[i], rl - 1, mc ? SP_FANOUT / 2 : 0))
                return 0;
            bsum = spN_sumbytes(n->children[i], 0, spN_cc(n->children[i]));
...
SP_STATIC int sp_checktree(const sp_Tree *t) {
    ...
    if (!sp_checknode(&t->root, t->levels, t->levels ? 1 : 0)) return 0;
    bsum = spN_sumbytes(&t->root, 0, spN_cc(&t->root));
 ...
 * segments before the current one; virtual tail states (off >= bytes)
 * carry no prefix semantics, only poff == 0 */
...
SP_STATIC int sp_checkcursor(sp_Cursor *C, size_t expected_off) {
    ...
    if (C->off >= C->tree->bytes) {
        check(C->poff == 0, "[chk] VIRTUAL poff=%lu off=%lu bytes=%lu\n",
              test_lu(C->poff), test_lu(C->off), test_lu(C->tree->bytes));
        return 1;
    }
    ...
    check(C->poff <= p->bytes[i], "[chk] POFF out of bounds poff=%lu len=%lu\n",
          test_lu(C->poff), test_lu(p->bytes[i]));
    return 1;
}
...
SP_STATIC sp_Node *leafV_(sp_State *S, ...) {
    ...
    for (i = 0; i < cc; i++) {
        ...
        spL_setid(n, i, id), n->bytes[i] = len;
    }
    va_end(ap);
    return n;
}
